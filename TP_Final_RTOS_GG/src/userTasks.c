		...
		for (i = 0; i < DACNUM; i++)
		{
			...
			{	//si hay un dato en la cola del canal "i" del DAC
				...
				case 0:
				{
					...
					DACData.mode = 0x40;	//modo de funcionamiento '01000000'
		...
		for (i = 0; i < (ADCNUM); i++)
		{
			...
			{ //si se tomó el semaforo del canal "i" del ADC
				...
				case 0:
					...
					adsData.modeHi = 0xC2; //modo de funcionamiento parte alta '11000010' CH0
					adsData.modeLo = 0xE3; //modo de funcionamiento parte baja '11100011'
					adsData.comState = dataSet_state; //Estado de la comunicación
					break;
				case 1:
					adsData.i2cNumber = LPC_I2C0;		//numero de puerto I2C
					adsData.i2cSlaveAddress = 0x48; //direccion del esclavo en el bus I2C '1001000'
					adsData.modeHi = 0xD2; //modo de funcionamiento parte alta '11010010' CH0
					adsData.modeLo = 0xE3; //modo de funcionamiento parte baja '11100011'
					adsData.comState = dataSet_state; //Estado de la comunicación
					break;
				case 2:		//los demas canales no estan implementados en eta muestra
					break;
				case 3:
					break;
				case 4:
				...
				if (xSemaphoreTake(I2CMutx,(TickType_t) 1 * portTICK_PERIOD_MS)== pdTRUE) //puedo esperar hasta 1 ms
				{
					...
					{
						//si el bus I2C esta disponible
						while (ads1115Read(&adsData) == FALSE); //pido el valor de la muestra al ADC
						xSemaphoreGive(I2CMutx); //Libero el I2C
